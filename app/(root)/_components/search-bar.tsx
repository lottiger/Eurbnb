...
interface SearchBarProps {
  onSearch: (destination: string, dates: string, guests: number) => void; // Funktion som körs vid sökning
}

const SearchBar: React.FC<SearchBarProps> = ({ onSearch }) => {
  const [destination, setDestination] = useState('');
  const [dates, setDates] = useState('');
  const [guests, setGuests] = useState(0); // Initialisera gäster till 0
  const [isGuestSelectorVisible, setIsGuestSelectorVisible] = useState(false); // Hantera visning av GuestSelector

  const dropdownRef = useRef<HTMLDivElement>(null);

  // Stäng dropdown när man klickar utanför den
  useEffect(() => {
  ...
  const handleSearch = () => {
    onSearch(destination, dates, guests); // Kör funktionen som skickas in via props när användaren söker
  };

  const handleGuestUpdate = (adults: number, children: number, infants: number) => {
    const totalGuests = adults + children + infants;
    setGuests(totalGuests); // Uppdatera antal gäster
  };
    ...
    <div className='shadow-md rounded-[50px] flex py-[11px] px-[34px] text-[14px] items-center'>
      ...
      <div className='relative px-[25px]' ref={dropdownRef}>
        ...
        <p
          ...
          onClick={() => setIsGuestSelectorVisible(!isGuestSelectorVisible)} // Visa/ dölj GuestSelector
        >
        ...
        {isGuestSelectorVisible && (
          <div className="absolute top-[68px] -left-[155px] bg-white rounded shadow-md z-50">
            <GuestSelector onGuestUpdate={handleGuestUpdate} onClose={closeGuestSelector} />
          </div>
        )}
      </div>
